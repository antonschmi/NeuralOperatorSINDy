...
%  ome(A) = -beta*A^2
%
% Unchanged from Champion et al. (2019) kpchamp/SindyAutoencoders/rd_solver/,
% except the final save() path, which now writes directly into
% experiments/reaction_diffusion/ (one level up from this script) so the
% Python loader (training_data.py) finds it without a manual file move.
