...
def sindy_simulate(z0, t, Xi, sindy_layer):
    """
    Integrate dz/dt = Theta(z) @ Xi from z0, building Theta the same way as
    sindy_layer (a FastSINDyLibraryLayer) does internally -- i.e. using its
    trained power_matrix/add_sine settings -- so the simulated trajectory
    matches what the model actually learned.
    """
    n = z0.size
    power_matrix = sindy_layer.power_matrix.detach().cpu().numpy()
    add_sine = sindy_layer.add_sine
    Xi = Xi.detach().cpu().numpy() if isinstance(Xi, torch.Tensor) else np.asarray(Xi)

    def f(z, t):
        z = np.asarray(z).reshape(1, n)
        theta_poly = np.prod(z[:, None, :] ** power_matrix[None, :, :], axis=2)
        if add_sine:
            theta = np.concatenate([theta_poly, np.sin(z), np.cos(z)], axis=1)
        else:
            theta = theta_poly
        return (theta @ Xi).reshape(n)

    return odeint(f, z0, t)

def get_lorenz_data(n_ics, noise_strength=0, linear=True):
    """
    ...
    data = generate_lorenz_data(ics, t, input_dim, linear=linear, normalization=np.array([1/40,1/40,1/40]))
